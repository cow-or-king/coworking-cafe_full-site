...
const CACHE_CONFIG = {
  ...
  maxAge: process.env.NODE_ENV === 'development' ? 5 * 60 * 1000 : 24 * 60 * 60 * 1000,
...
export function useDashboardData(): DashboardHookState {
  ...
  const [state, setState] = useState<DashboardHookState>(() => {
    ...
    if (globalDashboardData) {
      console.log(`📦 DASHBOARD GLOBAL CACHE HIT:`, Object.keys(globalDashboardData))
      return {
    ...
    if (cachedData && isCacheValid(cachedData)) {
      console.log(`📦 DASHBOARD LOCALSTORAGE CACHE HIT:`, Object.keys(cachedData.data || {}))
  ...
  useEffect(() => {
    ...
    const cachedData = getCacheData()
    if (cachedData && isCacheValid(cachedData)) {
      console.log(`📦 DASHBOARD CACHE HIT IN EFFECT:`, Object.keys(cachedData.data || {}))
      globalDashboardData = cachedData.data
      globalIsLoading = false
      globalError = null
      setState({
    ...
    const fetchData = async () => {
      try {
        ...
        console.log(`📊 DASHBOARD DATA SUCCESS - Ranges loaded:`, Object.keys(result.data))
