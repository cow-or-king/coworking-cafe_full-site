...
class DashboardCacheManager {
  ...
  private isCacheValid(cacheData: CacheData): boolean {
    ...
    const maxAge = process.env.NODE_ENV === 'development' ? 5 * 60 * 1000 : 24 * 60 * 60 * 1000
  ...
  private async fetchFromApi(): Promise<void> {
    try {
      ...
      console.log('📊 SINGLETON SUCCESS: Ranges loaded:', Object.keys(result.data))
...
export function useRangeData(range: string, compareRange?: string) {
  ...
  return useMemo(() => {
    if (!allData || isLoading) {
      ...
      console.log(`🔥 SINGLETON RANGE RESULT: useRangeData(${range}) returned:`, result)
      return result
    }
    ...
    console.log(`📊 SINGLETON RANGE DATA [${range}]:`, { mainData, compareData })
